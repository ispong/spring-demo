...
@RestController
@RequestMapping("/dbx")
public class TestDorisController {

    public void test(ISourceDTO source, SqlQueryDTO queryDTO, IClient client) {

        StopWatch stopWatch = new StopWatch("测试doris数据库");
        ...
        Boolean tableExistsInDatabase =
                client.isTableExistsInDatabase(source, queryDTO.getTableName(), currentDatabase);
        stopWatch.stop();

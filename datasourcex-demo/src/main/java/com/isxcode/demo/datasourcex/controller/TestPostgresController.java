...
public class TestPostgresController {
    ...
    @PostMapping("/open/postgres")
    public void testPostgres(@RequestBody DbxReq dbxReq) {

        IClient client = ClientCache.getClient(DataSourceType.PostgreSQL.getVal());

        PostgresqlSourceDTO source =
                PostgresqlSourceDTO.builder()
                        .url(dbxReq.getUrl())
                        .username(dbxReq.getUsername())
                        .password(dbxReq.getPassword())
                        .schema(dbxReq.getSchema())
    ...
    public void test(ISourceDTO source, SqlQueryDTO queryDTO, IClient client) {
        ...
        StopWatch stopWatch = new StopWatch("测试postgres数据库");
